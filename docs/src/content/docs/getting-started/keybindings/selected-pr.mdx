...
Set `defaults.prQuickApprove: true` to skip the prompt and approve immediately with
`defaults.prApproveComment` as the comment. With `defaults.prQuickApproveAdvance: true` the
selection also moves to the next PR after approving. See [Defaults][defaults] for details.

After approving (or any other action that changes a PR), the PR is re-fetched from GitHub so the
table and preview reflect its current state.

[defaults]: /configuration/defaults/#quick-approve-prquickapprove
