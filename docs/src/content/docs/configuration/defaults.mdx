...
defaults:
  ...
  prQuickApprove: false
  prQuickApproveAdvance: false
  preview:
    open: true
    width: 0.45
    height: 0.60
...
(for top or bottom position), and its position relative to the main content.
...
Specifies the height of the preview pane when it appears above or below the main content (in top or bottom
...
| :----- | :--------------------: | :-----: |
| String | `right`, `bottom`, `top`, `auto` | `auto`  |
...
- `top` — the preview pane appears above the search and results.
...
The mouse wheel scrolls the pane under the pointer. Click a result row to select it.
Mouse selection pauses while search, confirmation, or comment inputs are focused.
...
### Quick Approve (`prQuickApprove`)

| Type    | Default |
| :------ | :-----: |
| Boolean |  false  |

When enabled, approving a PR submits the approval immediately with
`prApproveComment` as the review body instead of opening the comment prompt.
Set `prApproveComment` to an empty string to approve without a comment.

The PR is re-fetched right after the approval (as after any other PR action),
so the review status in the table and the sidebar update without waiting for
the next refetch interval.

### Advance After Quick Approve (`prQuickApproveAdvance`)

| Type    | Default |
| :------ | :-----: |
| Boolean |  false  |

Only applies when `prQuickApprove` is enabled. After approving, the selection
moves to the next PR in the section so you can review a queue of PRs by
pressing the approve key repeatedly. On the last row the selection stays put.
